  ...
  public function finalizeDepartmentDocument($after_id = 0) {
    ...
    foreach ($nodedata as $d7id => $data) {
      ...
      if (!empty($data['path'])) {
        ...
        $local_destination = str_replace('legacy/development-services/pdf/news/meetings/2015', '', $local_destination);
...
//      }
   ...
   * @param $after_id (Node ID to resume after).
   *
   * @usage import:resource
   */
  public function finalizeResource($after_id = 0) {
    $nodedata = [];

    // Read extra field data for manual creation/update.
    if ($file = fopen($this->extensionList->getPath('if_sdmigration') . '/migration_files/nodes/resource.csv', 'r')) {
      fgets($file);
      while ($data = fgetcsv($file)) {
        $nodedata[str_replace('`', '', $data[0])] = [
          'department' => explode('|', str_replace('`', '', $data[1])),
          'address' => str_replace('`', '', $data[23]),
          'updated' => str_replace('`', '', $data[4]),
          'image_department' => str_replace('`', '', $data[6]),
          'image_license' => str_replace('`', '', $data[5]),
          'image_alt' => str_replace('`', '', $data[7]),
          'image_d7id' => str_replace('`', '', $data[8]),
    ...
    foreach ($nodedata as $d7id => $data) {
      ...
      if (!empty($data['address'])) {
        $address = explode('&#44; San Diego&#44; CA ', $data['address']);
        $street = str_replace('&#44;', ',', $address[0]);
        $zip_code = trim($address[1]);
        $node->field_address = [
          'country_code' => 'US',
